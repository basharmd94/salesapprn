// filepath: f:\2FastAPI\salesapprn\app\(tabs)\send-orders.jsx
...
export default function SendOrders() {
  ...
  // Define the showToast function before using it in other hooks
  ...
  // Reset loading state when component unmounts or path changes
  useEffect(() => {
    // Ensure mountedRef is true when component mounts
    mountedRef.current = true;
    
    return () => {
      // Reset loading state when navigating away
      setLoadingState({ sending: false, currentOrderId: null });
  ...
  useEffect(() => {
    // Make sure the ref is set to true when the component mounts
    mountedRef.current = true;
    ...
    if (isInitialLoad) {
      const loadData = async () => {
        try {
          ...
          // Only update ready state if component is still mounted
          if (mountedRef.current) {
            setIsReady(true);
            setIsInitialLoad(false);
    ...
    return () => {
      // Important: Mark the component as unmounted
      mountedRef.current = false;
    };
  }, [pathname, showToast, isInitialLoad, loadOrders]);

  const sendOrder = useCallback(async (order) => {
    // Store the order ID to reset in case the component unmounts
    const orderId = order.zid;
    
    // Set the loading state for this specific order
    setLoadingState(prev => ({ ...prev, sending: true, currentOrderId: orderId }));
    ...
    try {
      ...
      // Remove the sent order from local storage
      ...
      // Always reset the loading state if component is still mounted
      if (mountedRef.current) {
        setLoadingState(prev => ({ ...prev, sending: false, currentOrderId: null }));
      }
    }
  }, [orders, loadOrders, showToast, mountedRef]);
  ...
  const deleteOrder = useCallback(async (order) => {
    try {
      ...
      // Only reload orders and show toast if component is still mounted
      if (mountedRef.current) {
        await loadOrders();
  ...
  const handleBulkSend = useCallback(async () => {
    ...
    setLoadingState(prev => ({ ...prev, sending: true, currentOrderId: null }));
    ...
    try {
      ...
      // Only close dialog if mounted
      if (mountedRef.current) {
        handleClose();
      }
    }
  }, [orders, loadOrders, showToast, handleClose, mountedRef]);
  ...
  }, [orders, loadOrders, deleteOrder, showToast, mountedRef]);
  ...
  }, [orders.length, loadingState.sending]);
